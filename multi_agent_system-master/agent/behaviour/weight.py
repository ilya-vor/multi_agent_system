    ...
    async def run(self):
        ...
        if (msg and
            ...
            try:
                ...
                # Сохраняем известную информацию о соседе для последующего выбора
                try:
                    jid = str(msg.sender)
                    self.agent.known_neighbors.setdefault(jid, {})
                    self.agent.known_neighbors[jid]["specialization"] = neighbor_specialization
                    self.agent.known_neighbors[jid]["last_weight"] = neighbor_weight
                    self.agent.known_neighbors[jid]["task_count"] = neighbor_task_count
                except Exception:
                    pass
                ...
                if self.agent.my_weight > neighbor_weight + threshold_value:
                    ...
                    if compatible_for_neighbor:
                        ...
                        # Попробуем найти объект, который по профессии подходит получателю,
                        # даже если он тяжелее, чем weight_to_shed
                        compatible_any_weight = [item for item in self.agent.backpack if "allowed_professions" in item and neighbor_specialization in item["allowed_professions"]]
                        if compatible_any_weight:
                            self.agent.transfer_object = min(compatible_any_weight, key=lambda x: abs(x["weight"] - weight_to_shed))
                        else:
                    ...
                    # Если выбранный объект не подходит по специализации для соседа — отменяем передачу
                    if ("allowed_professions" in self.agent.transfer_object and
                            neighbor_specialization not in self.agent.transfer_object.get("allowed_professions", [])):
                        print(f"{get_time()} [WeightReplyBehaviour] {self.agent.jid}: Отмена передачи: объект не подходит по специализации для {self.agent.neighbor_choice}")
