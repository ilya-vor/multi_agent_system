...
# Ensure repository root is on sys.path so `from agent.agent import ...` works
    ...
    try:
        ...
        while True:
            ...
            if current_time - last_save >= args.save_interval:
                if worker.save_backpack():
                    print(f"{get_time()} [PERIODIC SAVE] {worker.jid}: Периодическое сохранение выполнено")
                else:
                    print(f"{get_time()} [PERIODIC SAVE] {worker.jid}: Ошибка периодического сохранения")
                last_save = current_time

    except KeyboardInterrupt:
        print("Остановка...")
        if worker.save_backpack():
            print(f"{get_time()} [FINAL SAVE] {worker.jid}: Финальное сохранение выполнено")
        else:
            print(f"{get_time()} [FINAL SAVE] {worker.jid}: Ошибка финального сохранения")
        await worker.stop()
